...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({
      ...
      note: "NS: OSM+NSTDB. NB: OSM+Forest Roads. ON: OSM+MNR resource (capped). AB: OSM+Access Roads. BC: OSM+DRA resource/trail (capped). QC/PE/MB/SK: OSM-only (SK permanent). No NRN in adventure packs."
    });
  }
