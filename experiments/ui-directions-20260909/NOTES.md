...
Bottom navigation restored as toggles: active item closes its sheet; another switches sections. Route remembers its last tool and route state. Active navigation is solid orange/white; a built route gets an orange outline when its tools are closed, including while another sheet is open. Map-only state hides fuel/preferences and retains right-side map controls. HTML only.
