...
## New central NS closed-loop stress case — stable f2da612 protected

New fixture bench/fixtures/central-loop-20260909.json: Porters→Yarmouth→44.660761,-65.520139→Porters, Balanced/Dirt/Balanced, known access,180km usable. Local and public hosted first2legs complete530.220/220.976km with0within-legrepeat. Return leg fails core dirt-10/dirt-30 label limits; hosted mixed-2 also reaches deadline. Public sourcef2da612 verified.225km local variant also completes first2 then fails core label limits. Exact180return request on previousstable4da3822 also fails, so no rollback of qualified snapping.

Independent interval audit across the first2 same-NS-graph legs finds152571m shared road despite0internalrepeat perleg. This is NOT a complete loop nor proof all shared road is avoidable; retain whole-itinerary history limitation explicitly. First180return carries67842.419m remaining and59historyedges. Diagnostic-only shortening to last3historyedges preserves inferred arrival direction but still fails dirt-10/dirt-30/mixed-2 label limits locally8.799s, so recent-road avoidance alone is not the cause. Empty/one-edge-history probes are invalid arrival contracts and provide no routing comparison. These probes were never deployed; runtime unchanged.

Evidence /tmp/dirt-central-loop{180-local,180-hosted,225-local}, /tmp/dirt-central-loop180-{return-request.json,previous/response.json,short-history-response.json,overlap.json}.6local route attempts +3hosted chain attempts +1previousstable request; each3legchain has2pass/1fail, not blanket passes. Bench fixture records known failure. Next useful investigation: fuel-state label growth with this shorter single-region return request, alongside existing freshreverseInverness failure. No running tests, no pending publication, no runtime/native/pack changes. Keep f2da612 stable and heartbeat active before08:00Halifax.

## Central return fuel frontier trace — no runtime change

Saved180km return request instrumented locally at existing400000-label cap. Across six failed initial/retry searches:220498–225295 expanded,156088–162598 frontier keys; every expanded label had0retrace/0urban avoidance, and more than99% had already refuelled. Guidance3 still hits the same cap. Evidence /tmp/dirt-central-loop180-trace.err. This confirms broad post-refill fuel-state growth, not a single huge frontier or recent-history-only effect.

Diagnostic-only removal of onward retrace priority still leaves dirt-10/dirt-30 at label_limit; mixed-2 becomes feasible but retains227m repeats, paved94m. Whole request staysunknown,9.465s local. Thus zero-retrace lexicographic ordering is not sufficient to explain/fix the core search failure; do not publish that weakening. Evidence /tmp/dirt-central-loop180-no-retrace-priority-response.json. All trace and priority edits fully restored byte-for-byte; gitdiff clean before this note. No hosted tests warranted for rejected local experiment. Stablef2da612 unchanged, no pending owner/deploy action.

Next investigation should target fuel-aware admissible COST guidance or exact resource dominance, preserving physical remaining-fuel labels. Prior nearest-fuel/destination distance pruning and uncapped cost bounds alone were already ineffective; do not repeat them unchanged. Do not quantize away feasible fuel labels, raise400000cap, weaken sharedpool, or disable repeat priority to manufacture passes. No running process. Overnight heartbeat remains active until08:00Halifax.

## Short Eastern Shore closed-loop qualification — waypoint turnaround flag

New benchfixture eastern-short-loop-20260909.json exercises90km usable, knownaccess, Clean/Balanced/Dirt coarsezoom6.6 and reverse Dirt/Balanced/Clean zoom11. All6local+6hostedlegscomplete with full sharedpool, fuelcarry, exact adjacent joins and exact loopclosure. Exactpublicf2da612/Atlantic02 independently asserted. Forward4.000/19.789/31.674km; reverse22.089/28.018/4.117km. Hosted server times0.605–6.954s. No generatedfuelstops. Evidence /tmp/dirt-eastern-short90-{local,hosted}, /tmp/dirt-eastern-short90-reverse-{local,hosted}.

One departure shape remains flagged in BOTHdirections:734.324m forward from riderprojectionfraction0.55629958 on w156997578:58613:58562 to node58562, reverse the same734.324m, then pass back through pin toward58613. All4candidates share thisrepeat; not a fuel-stop detour. Raw node58562has3incidentedges, so do NOT call it a degree1deadend:69602verified residential,69605verified residential,69879unknowntrack. Existing no-mid-edge-Uturn/exactarrival behavior sends the rider to a junction for reversal. Alternative route quality at this waypoint is not adjudicated; physical acceptance notclaimed. Evidence /tmp/dirt-eastern-short90-deadend.json (filename historicalprobe; contents showdegree3). No runtimechange/deployment.

No runningtests. Stablef2da612 protected. Further useful work is qualified rider-waypoint turnaround semantics or exact fuel-state search guidance; no unsuppliedhistory guessing. Overnight heartbeat remains active before08:00Halifax.

## Exact retreat ancestry deduplication experiment — rejected locally

Frontier trace on savedcentral180return found146422–151405active labels with retreat cursors among178792–197597active labels; no full fuelApproach state. Exact directed-road ancestry interning (ignoring fuel actions only) reduces distinct diagnostic state keys156088–162598 to121396–133411, confirming duplicated ancestry but not sufficient proof of runtime gain. Evidence /tmp/dirt-frontier.err and /tmp/dirt-canonical-trace.err.

Implemented a local-only lazy WeakMap/structural-Map canonical ancestry key, budget-charged, preserving arc identity/direction, turn state, remaining fuel and full reverse-road sequence.35search/fuel tests pass. Savedcentral180return still fails all nonpaved core fuel searches atlabel_limit,9.840s local; no demonstrated end-to-end improvement. Do not publish added memory/work without a recovered case or measured gain. All runtime edits reverted byte-for-byte; gitdiff clean before this record. Prototype retained only /tmp/dirt-canonical-experiment-resource-search.js, evidence /tmp/dirt-canonical-experiment-{response.json,tests.log}. No hosted tests/deployments/nativechanges.

Stablef2da612 remains protected. Exact ancestry duplicates alone do not resolve broad post-refill search growth; do not repeat this experiment unchanged. Fuel-aware admissible cost guidance remains unexplored. No running processes or pendingowner action. Heartbeat active before08:00Halifax.

## NB coastal120km mixed-style range stress — distinct fuel and waypoint flags

New fixture nb-coastal-range-20260909.json. ForwardClean/Balanced zoom7 and reverseDirt/Clean zoom10, knownaccess120km usable. All4local+4publichosted legscomplete with fullcorepool, fuelcarry/escape and exact primaryjoins, exactf2da612/Atlantic02 asserted. Forward112.820/187.925km, reverse176.451/157.600km; hosted12.219/2.815/4.516/3.048s. Completion is NOT shapeacceptance.

ForwardClean firstleg retains6892m repeated pavedroad after automaticstationosm:n5300882192 at47.006429,-65.41969. Paved approachrefinement reportsno_improvement; nonpaved sharedcandidates0repeat. Same exactfirstrequest with225km initial/usable produces0stops/0repeat locallyANDhosted (independentexactsource/fullpool/fuel/shape checks). Low-rangefuel placement is therefore a distinct unresolvedshape, not an off-roadpin issue. Do not claim the120km detour unavoidable without an alternative-station proof.

Middlepin47.047134,-64.891699 departure retains3026.015m immediate out/back in bothdirections, beforefuelstop. ReversefirstDirt0repeat. Treat rider-turnaround andfuelapproach separately. Evidence /tmp/dirt-nb-coastal120-{local,hosted,reverse-local,reverse-hosted}, /tmp/dirt-nb-coastal225-first-{request.json,response.json,hosted}. Thisbatch5local+5hostedrequests, allcomplete, with3shape-flagged120km legs. No runtimechange/deploy, no production/native/GitHubchanges. Stablef2da612 protected.

No runningprocesses. Next useful targetedfuelcase is savedfirstClean120request, whose225control is clean. Existing30kmhistory/per-pin precision and turnaround semantics remain nativecontract concerns. Overnightheartbeat activebefore08:00Halifax; prepare morning handoff without representing all automatedcompletions as route-qualitypasses.

## NB Clean120 fuel-detour diagnosis — stable unchanged

Normal-range control directroute73.299km plus required destinationfuelescape58.622km exceeds120km initialusable; the selected smaller-range request needs a refill under the existing destination-escape rule. Selectedstoposm:n5300882192 leaves58.624km arrivalfuel for58.622km provedescape. This is model feasibility, not current pump-access/availability verification.

Diagnostic-only removal of that automaticstation from the local stationcatalog yields a complete alternate Clean route115.657km viaosm:n3863096690, but repeatedroad worsens6892→13904m. Thus station replacement alone is not a demonstrated fix; no catalog/runtime edit was retained or published. Original sharedpool zero-repeat nonpaved candidates contain26.44%dirt versus selectedClean2.54%dirt, so simply selecting one would materially change Clean character. Neither experiment proves the originaldetour globallyoptimal/unavoidable. Evidence /tmp/dirt-nb-coastal120-{first-request.json,exclude-response.json,exclude.log}; temporaryloader /tmp/dirt-replay-exclude-nb-pump.js only.

No runningwork, no pendingpromotion. Stablef2da612 protected. Draft concise morning report in docs/OVERNIGHT-ATLANTIC-20260909.md; update with any subsequent qualified work, deliver/pauseheartbeat at08:00Halifax. Remaining major work is fuel-state search scalability and native history/per-pin/turnaround contracts, not blanket removal of fuelstops or weakening Clean.

## Overnight sweep handoff — automation paused early after completed batch

Final savedcentral180CPU profile (no runtime edits) attributes approximately19.9% sampledtime tolabel insertion/dominance,12.6% GC,11.7% turntransition and9.4% mainsearch; budgetchecks8.2%. Evidence /tmp/dirt-central-fuel.cpuprofile and /tmp/dirt-central-fuel-profile-summary.json. This identifies potential performance work but does not resolve the400000label limit; not a reason to publish unqualified changes.

Overnight automated sweep concluded before08:00 under heartbeat's explicit early-completion clause. Heartbeat overnight-atlantic-routing-refinement PAUSED via automation tool. No active processes or pending deployments. Qualified stablef2da612/Atlantic02 remains, rollback4da3822 preserved. All rejected runtime experiments restored; no native/production/GitHub changes. Morning report docs/OVERNIGHT-ATLANTIC-20260909.md is final for this sweep. Remaining defects are explicit, not claimedfixed: central/reverseInverness fuel search, whole-itinerary overlap, riderturnarounds, smaller-range Cleanfuel detour and narrow coldheadroom. User physical acceptance pending.
