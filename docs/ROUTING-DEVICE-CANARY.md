# Current stable DEV — connected-road snapping, 9 September 2026

Stable `https://pack-fabric.vercel.app` resolves to EXACT `pack-fabric-qu663xg1q`, source `f2da612ae73dd175123430391010b48f0d9408a7`, runtime `0496982eb044d0504763f175aba00f7b59ec319f`. Both Atlantic packs remain `fabric-v4-20260908-02`, flag `ns-nb-v1`; all five region02 settings unchanged. Rollback `4da3822dd7d37f6dde7720d61668aba1d7e883f8` / `pack-fabric-bz4kvcmdt` retained. Production, native app and phone packs unchanged.

Ordinary start pins can now recover a connected eligible road when nearby isolated roads crowd the initial candidate list. Recovery stays inside the allowed snap radius, preserves valid near pairs and destination projections, and never moves a fixed fuel start or overrides incoming arrival history. No artificial off-road connector is added. Includes previously qualified continuation recovery, wider area snapping, fuel-repeat guards and performance improvements. Saved built geometry remains unchanged.

197 adventure +22 topology tests and22 private hosted requests pass. Two real NS cases recover starts500.17m and3205.16m away; all three styles pass with zero within-leg repeat and identical shared candidate pools. Six baseline geometries and fuel-stop IDs remain exact. Eight independent public checks pass: both snap cases plus original and reversed three-leg coarse-pin routes. Exact source/releases, full candidate pool, fuel and geometry checks passed. Evidence `/tmp/dirt-isolated-private-*` and `/tmp/dirt-isolated-public-*`.

Flags: fresh reversed Inverness180km still reaches the dirt-30 label limit. Cold Clean headroom remains narrow. Whole-itinerary overlap beyond native30km history, persistent per-pin precision, Clean departure after unknown-access arrival, the291.77m rider-dead-end return, and explicit endpoint-refill qualification remain open. Automated qualification does not imply physical acceptance or release readiness. No physical input required overnight.
