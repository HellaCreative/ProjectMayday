...
| Current Ontario baseline | `.build/recovery-evidence/balanced-page-capacity`, current immutable pack, southern Ontario endpoints, fuel off, optimized simulator build with coverage explicitly disabled. Smaller Balanced-only pages avoid the previous128MiB payload exhaustion: first/repeat18.299/18.310s return a complete road but timedOut=true, still failing qualification. Search label payload peaks at73.6MiB, with about726,000 finite entries among1.61million allocated slots; directory logical payload is separately measured at0.77MiB and excludes object/allocator overhead. Sampled process footprint284.3/271.4MiB. Dirt31.481/14.045s remains complete. Clean retains the scoped-bounds speedup; exact results and source patch are in the evidence folder. The12-test run has two Balanced qualification issues; focused allocation/pointer/native equivalence tests pass. No larger cap or altered profile threshold was used. |
...
| Current four-owner qualification | `/tmp/Dirt-Lazy-Destination-Fuel-20260914.xcresult`:11tests in three suites pass, including all four exact owner requests, first-use/repeat, source/cancellation checks, and native retained-incumbent validation. Owner calculations take20.847,13.225,13.269,15.535s, sampled footprints127.1,90.8,87.8,87.7MiB. All complete route responses match `.build/recovery-evidence/scoped-bounds-integrated` apart from search timing, including geometry, dirt and refill identities. These are simulator results, not broad regional or physical-device acceptance. |
| Latest focused work | Initial-station bounds retain one completed vector under exact origin, ordered station coordinates/radii, incumbent distance, field limits and source identity. Pack/index ownership is weak; stale data, incomplete results and cancellation cannot publish a hit. The 256 KiB limit is retained logical cache payload, not total process memory or caller-held results. Actual station approach selection remains unchanged. Destination fuel is still screened lazily; memory-limited Balanced completion remains honestly marked as unfinished search. Pack bytes and approved UI are unchanged. |
| Rejected performance experiment | `.build/recovery-evidence/ancestry-index-fourth`: a bounded per-pop ancestry table preserved focused route equivalence but slowed the actual first pump calculation to about 12.8 s from about 8 s. It was removed from active source; staged prototype and measurements are retained. Instrumentation observed about 302 million predecessor visits before this experiment. The active fixed 4 KiB label-page read cache reduces lookup overhead without changing search decisions; predecessor traversal itself remains. |
