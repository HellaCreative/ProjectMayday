...
| Existing native candidate | `.build/engine-architecture`, branch `audit/baseline-recovery-20260913`, Recovery checkpoint `efed142` plus active save, seam-frontier, destination-escape and label-lookup repairs. Verify status before editing. |
...
| Owner-log replay | All four exact build-42 requests now pass with initial fuel and actual arrival-aware destination escape: `/tmp/Dirt-Journey-Checkpoint-20260913.xcresult` (13 tests in three suites, including saved-route and legal-continuation tests). Current catalog, coordinates/seeds, 200 km / 10% settings and outputs are preserved in `.build/recovery-evidence/journey-checkpoint`. Escape ordering and verified unrestricted same-road reversal repair the earlier timeout. This is simulator qualification of these requests, not phone or broad regional acceptance. |
