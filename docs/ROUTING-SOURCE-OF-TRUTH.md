...
| Existing native candidate | `.build/engine-architecture`, branch `audit/baseline-recovery-20260913`. Scoped-bounds checkpoint `cfad1d7` and earlier `aa4e348` remain recoverable. The following recovery checkpoint adds smaller Balanced label pages, preserves a validated completed road with an honest warning after label exhaustion, and lazily screens destination fuel. It is not the final qualified candidate; inspect status before editing. |
...
| Latest first-owner cold/repeat replay | `/tmp/Dirt-Lazy-Destination-Fuel-20260914.xcresult`: exact first owner request passes at20.286/6.030s, sampled footprints81.3/71.1MiB on the named M1 simulator. Previously24.383/9.744s with eager destination screening. Routes and refill identities are unchanged apart from search timing. First-use means fresh application preparation, not an empty OS file cache. Latency targets remain unmet. |
...
| Latest focused work | Destination escape now prepares one bounded relaxed field, visits stations in the same existing order, and stops after the first actual arrival-aware route fits remaining range. Source/cancellation is checked before/after screening and actual proof. The first-owner final escape screens one station instead of all671. Initial closest-station proof is unchanged. Balanced memory exhaustion now uses normal reconstruction/legal validation for an existing destination candidate and discloses unfinished selection; no incumbent/cancellation remains failure. New tests pass in the11-test lazy-destination run. Packs and approved UI are unchanged. |
