...
| Bounded station-order preparation, not yet enabled | Source-paired private-index sampling now prepares ordering hints from completed road-distance fields while retaining every station, including unsampled/unknown rows. It samples at most 64 parent memberships per station per field across nearby spatial cells; samples never prove access, range, progress or a fuel gap. Actual station matching/routing remains mandatory. Eleven tests in three suites pass in `/tmp/Dirt-Lazy-Spatial-Sampling-R1-20260914.xcresult`, including cancellation, changed/wrong source pairs, late-ID roads and the exact first-owner post-initial-refill preparation. Hint work falls from 18.6 million parent memberships and 11.693/11.205 seconds to 85,332 memberships and 0.666/0.639 seconds; cold preparation including decoded graph/index plus one exact station match totals 2.699 seconds. All 671 stations remain represented; 434 have both hints and eight neither. The first refined station is unchanged at 45.667087,-62.34412. Sampled footprint 82.5/81.5 MiB, RSS 426.6/427.6 MiB on the named M1 simulator. This API is unactivated: it does not qualify a complete fuel chain or selective core loading. Evidence: `.build/recovery-evidence/lazy-spatial-sampling`. |
