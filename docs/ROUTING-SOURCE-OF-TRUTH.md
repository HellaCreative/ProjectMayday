...
| Existing native candidate | `.build/engine-architecture`, branch `audit/baseline-recovery-20260913`, Recoverable checkpoint `e9da9e9` includes private matching indexes, paged geometry, bounded preparation reuse, fuel corrections and honest search reporting; earlier checkpoint `95158d6` is preserved. Neither is the final integrated qualified candidate. Active continuation repairs remain under test. Verify status before editing. |
...
| Latest first-owner cold/repeat replay | `/tmp/Dirt-Fuel-Reuse-Integrated-20260913.xcresult`: exact first owner request passes at 25.990/9.604 s, sampled footprint 96.5/82.1 MiB on the named M1 simulator. Both runs have identical complete route responses and three refill identities. One previously verified exit is reused in each calculation; actual destination escape remains verified afterward. Earlier cache-first results were 32.010/12.097 s. Raw identity, phases and geometry are in `.build/recovery-evidence/fuel-reuse-integrated`. First-use is fresh application preparation, not an empty OS file cache. Targets remain unmet. |
| Current four-owner qualification | `/tmp/Dirt-Fuel-Reuse-Integrated-20260913.xcresult`: 17 tests in five suites pass, including all four exact owner requests and first-use/repeated preparation. In owner order the calculations take 25.754, 17.556, 15.143 and 17.302 s, with sampled footprints 120.1, 131.3, 101.0 and 92.4 MiB. Each includes initial refuelling, actual selected-stage range checks, preserved destination and arrival-aware legal destination escape. The fourth request uses the initial station plus one later refill; its previously rejected 4.08 km repeated exit is replaced by a same-anchor alternative and retained for the next stage. These are simulator request results, not broad qualification or physical-device acceptance. |
| Latest focused work | Fixed-size non-owning label-page read caches and cache-first exact station matching pass native equivalence tests. Source/settings/arrival-bound exit reuse now succeeds through the real builder: presentation connector IDs no longer invalidate native road-history identity, while their distances, downstream overlap checks and final fuel proof remain intact. `/tmp/Dirt-Fuel-Exit-Reuse-20260913.xcresult` passes ten tests; isolated fourth-request calculation takes 24.354 s (whole test 26.442 s), with 119.8 MiB sampled footprint. This deliberately changes the fourth generated ride by accepting a verified non-repeating onward alternative instead of backtracking to another pump. Pack bytes and approved interface are unchanged. Independent review found no connector-reuse correctness defect. |
