...
| Existing native candidate | `.build/engine-architecture`, branch `audit/baseline-recovery-20260913`. The scoped road-bounds recovery checkpoint follows rollback `aa4e348`; it reuses bounded validated index pages through one complete calculation, removes the unused V4 node-sized nil table, and measures predecessor visits locally. All four owner requests pass; this is not the final qualified candidate. The slower callback-based reverse-index experiment was removed; raw source/results are preserved. Inspect status before editing. |
...
| Current Ontario baseline | `.build/recovery-evidence/scoped-bounds-integrated`, current immutable pack, southern Ontario endpoints, fuel off, optimized simulator build with coverage explicitly disabled. Dirt first/repeat 29.095/13.717 s; Clean 3.275/3.079 s. Their full geometry and road legs match the preceding no-coverage baseline. Balanced fails with labelMemoryCap at 60.787/61.452 s after repeated search envelopes; it previously returned a timed-out candidate at about 18.3 s. This regression is unresolved and prevents qualification. Sampled footprints first/repeat: Dirt 300.9/211.4 MiB, Clean 250.1/219.7 MiB, Balanced 337.9/296.6 MiB. Full-region allocation and unnecessary initialized label slots remain under investigation. |
...
| Latest first-owner cold/repeat replay | `/tmp/Dirt-Scoped-Bounds-Integrated-20260914.xcresult`: exact first owner request passes at 24.383/9.744 s, sampled footprints 82.2/70.9 MiB on the named M1 simulator. Complete repeated route responses are identical; three refill identities and actual destination escape remain verified. First-use means fresh application preparation, not an empty OS file cache. Latency targets remain unmet. |
| Current four-owner qualification | `/tmp/Dirt-Scoped-Bounds-Integrated-20260914.xcresult`: all four owner requests pass at 25.237, 15.986, 13.557 and 15.248 s, sampled footprints 126.9, 91.8, 88.8 and 88.8 MiB. Compared with `.build/recovery-evidence/native-performance`, all route responses match apart from search timing, including road geometry, dirt contribution and refill identities. The complete run has 14 tests in three suites and two Ontario Balanced issues; it is not an integrated pass or physical-device acceptance. |
| Latest focused work | `/tmp/Dirt-Scoped-Road-Bounds-20260914.xcresult`: 11 tests in three suites pass. CPU sampling found town-avoidance bounds checks repeatedly validating and allocating tiny file reads. One scoped, source-validated page query now serves the actual public route and nested matching; closing validation occurs before return. Same-source ABBA Ontario Clean takes 3.177/3.132 s enabled versus 5.917/5.936 s disabled, with identical geometry, road IDs, metres and search work. Source mutation, nested lease budget, wrong owner, cancellation and three-profile avoidance equivalence tests pass. Pack bytes and approved UI are unchanged. |
