...
| Current cumulative Balanced audit | Source inspection confirms a remaining contract gap: request options and fuel/regional continuation carry road history and legal arrival, but no accumulated surface distances. Native resource searches restart their mix calculation at each subdivision. Balanced therefore does not yet qualify the owning rider-leg mix requirement across refills and borders. Repair must carry exact traversed surface totals separately from the current fuel-stage distance and include that context in cache identities; initial refill approach is excluded and ordinary rider-leg boundaries reset the mix. |
