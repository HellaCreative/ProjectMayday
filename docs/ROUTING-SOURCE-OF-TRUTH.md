...
| Paged continuation and policy parity | `/tmp/Dirt-Paged-Continuation-Policy-20260914.xcresult` passes nine tests in four suites. Unactivated paged continuation preserves node/fractional arrival, active via-way progress, exact restriction signatures and source epoch against the existing array reference; mismatched identity and expired queries reject. Shared Clean base-cost reads match native fixtures, including ferry early return, lazy read order, malformed dictionaries and cancellation. This extraction does not yet include the entire outer cost/access/town policy or activate paged app search. Raw exact source: `.build/recovery-evidence/paged-continuation-policy`. |
