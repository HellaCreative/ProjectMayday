...
| Connected-stage prototype | `/tmp/Dirt-Connected-Stage-Focused-20260914.xcresult`: fourteen tests in five suites pass. The unactivated recorded-node first Clean pass retains competing cost/length histories across exact regional seams under one stage cap; the fixture returns a 190 km legal chain within 207 km where a cheaper isolated prefix cannot fit its tail. Native full-road cost extraction matches reference fixtures across three profiles. Tests cover turn-state transfer, source changes, resource limits, seam-origin local routing and endpoint geometry validation. This does not yet support app tap/fractional matching, customer endpoints, all profiles or fallback orchestration; source graphs remain fully decoded. It is not a qualified replacement for current app regional routing. Exact source/results are in `.build/recovery-evidence/connected-stage-focused`. |
