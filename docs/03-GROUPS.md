...
Group ride list/detail, invite codes, presence sharing, and map roster. Spec: [WEB-SPEC-FOR-IOS.md](./WEB-SPEC-FOR-IOS.md) §4. Auth prerequisite: [04-PROFILES-AUTH.md](./04-PROFILES-AUTH.md).
...
**Not used on iOS yet:** `rider_alerts` (web table exists; native Report does not insert).
...
Web POC uses a private Realtime channel `group:{groupId}` with presence + broadcast (`location`, `alert`, `sharing_off`).

**iOS v1 does not subscribe to Realtime.** It:

1. Writes the same `rider_presence` rows the web reads.
2. Polls that table on a 10s timer while a group detail is open.
3. Publishes location on a 5s timer while sharing.

Cross-client interoperability with the web app works for **persisted** presence (list counts, pins, route-to-member). Latency and leave detection are coarser than Realtime. Broadcast alerts are absent.
...
2. Confirm table/RPC names against [WEB-SPEC-FOR-IOS.md](./WEB-SPEC-FOR-IOS.md) §4 — do not invent columns.
3. Check how rider markers merge with planner markers (`paintLiveRiders` / `refreshMap`).
4. **Invariants:** groups require signed-in session; invite codes stay 6-char lowercase alnum; live window 120s; closing sheet resets detail; do not break web’s `rider_presence` shape.
