...
| Province network overlays NS/NB/QC actually drawn | **Done** — corridor when route exists / zoom ≥12.5; lens via Show NS/NB/QC route lines |
| Layers toggles wire to overlay paint | **Done** — Map visibility removed; Route data = lens; corridor always available with a route |
...
| Surface toggle | HTML map-visibility drawer (optional) | **Removed** — network classes always paint when loaded |
| Province toggle | `networkOverlayProvince` / `showAllNetworkData` | `showNSLines/showNBLines/showQCLines` (mutually exclusive lens) |
| Corridor when toggle off | Always NS around route / detail zoom | **Done** — `NetworkOverlayManager` corridor (2–3 km + route anchors) |
| POI tap → route | `usePoiAsWaypoint()` | `confirmationDialog` → `planner.routeToCoordinate` |
| Min zoom | POI 8.5; network 12.5 (corridor) | POI 6.5 overview; network corridor ≥12.5 or active route; lens uses viewport |
| Chunk cap | POI 14; network 6 (corridor) | POI 14; network 6 corridor / 8 lens |
| Feature cap | web: CONTEXT_MAX_FEATURES=1600 | network: 1600 corridor / 5000 lens |
