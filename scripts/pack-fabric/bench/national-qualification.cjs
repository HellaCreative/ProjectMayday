...
(async()=>{
 ...
 const result=await adventureCanaryRequest(body,body.fuel?'fuel':'route',{environment:{DIRT_ADVENTURE_CANARY:'national-v1',DIRT_ZERO_REFILL_ADVISORY:process.env.DIRT_ZERO_REFILL_ADVISORY},load:async resolution=>{
  const rows=resolution.regionIds.map(id=>{
   ...
   const row={pack:decodeGraphV4(graph,geometry),geom:decodeGeometryV1(geometry),stations:JSON.parse(fuel).stations,identity:[{regionId:id,releaseId:require('node:path').basename(require('node:path').dirname(process.argv[2])),graphSha256:sha(graph),geometrySha256:sha(geometry),fuelSha256:sha(fuel)}]};
