...
for(const usable of [333000,162000])for(const allowUnknown of [false,true])for(const reverse of [false,true])for(const profile of ['dirt','balanced','clean']) {
 ...
 const result=buildRideAlternatives({dirtContinuityMeters:Number(process.env.REBUILD_CONTINUITY_METERS||0),input,pack,geom,stations,preferOnwardFuel:true,fuelHeuristicWeight:Number(process.env.REBUILD_CONTINUITY_WEIGHT||1),maxFuelLabels:Number(process.env.REBUILD_FUEL_LABELS||400000),additionalUrbanAreas:require('../routing/lib/adventure/nb-urban-review-20260908-01.json').cores,expandedCandidates:true,revision:JSON.stringify(identity),context,budget:createBudget({deadlineAtMs,maxExpansions:30000000}),preparationBudget:createBudget({deadlineAtMs,maxExpansions:20000000})});
