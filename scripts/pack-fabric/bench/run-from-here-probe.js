...
const directory=path.join(process.env.REBUILD_PACK_ROOT,"ns"),loadAt=performance.now();
...
const loadMs=Math.round(performance.now()-loadAt),preparationCache=createPreparationCache();
...
for(const c of cases.filter(c=>!process.env.REBUILD_FROM_HERE_CASE||c.id===process.env.REBUILD_FROM_HERE_CASE)) {
  const request={mode:"from_here",anchors:[{id:"start",lat:44.76484,lon:-63.34023},{id:"destination",...c.end}],
  ...
  try {result=buildFromHere({input:request,pack,geom,stations:c.noStations?[]:stations.filter(s=>s.id!==c.removed),revision,preparationCache,budget:createBudget({deadlineAtMs:Date.now()+20000,maxExpansions:maxWork}),
    objectiveId:`experimental-dirt-1-other-${ratio}`,edgeCost:a=>a.distanceMeters*(surfaceKind(a.surfaceLeaf)==="dirt"?1:ratio)});}
  catch(error){result={state:"error",reason:error.message,stack:error.stack};process.exitCode=1;}
  const row={case:c.id,source,loadMs,maxWork,result,elapsedMs:Math.round(performance.now()-at),processPeakRssKiB:process.resourceUsage().maxRSS};
  fs.writeFileSync(path.join(output,`${c.id}.json`),JSON.stringify(row));
