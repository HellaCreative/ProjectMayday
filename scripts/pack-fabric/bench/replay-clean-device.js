...
for(const avoidMotorways of (dalhousie?[false]:[false,true])) {
 ...
 const result=buildRideAlternatives({fuelSpurAlternatives:!!process.env.REBUILD_FUEL_SPURS,additionalUrbanAreas:process.env.REBUILD_NB_URBAN?require('../routing/lib/adventure/nb-urban-review-20260908-01.json').cores:[],input,pack,geom,stations:stations.filter(s=>s.id!==process.env.REBUILD_EXCLUDE_STATION),avoidMotorways,expandedCandidates:true,maxFuelLabels:100000,fuelHeuristicWeight:2,revision:JSON.stringify(identity),context,budget:createBudget({deadlineAtMs,maxExpansions:30000000}),preparationBudget:createBudget({deadlineAtMs,maxExpansions:20000000})});
