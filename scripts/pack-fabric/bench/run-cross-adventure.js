...
for(const usable of [333000,162000])for(const allowUnknown of [false,true])for(const reverse of [false,true])for(const profile of ['dirt','balanced','clean']) {
 ...
 const result=buildRideAlternatives({pavedFuelHeuristicWeight:process.env.REBUILD_PAVED_WEIGHT?Number(process.env.REBUILD_PAVED_WEIGHT):undefined,dirtContinuityMeters:Number(process.env.REBUILD_CONTINUITY_METERS||0),input,pack,geom,stations,avoidMotorways:profile==='clean',preferOnwardFuel:true,additionalUrbanAreas:require('../routing/lib/adventure/nb-urban-review-20260908-01.json').cores,expandedCandidates:true,maxFuelLabels:400000,fuelHeuristicWeight:2,revision:JSON.stringify(identity),context,budget:createBudget({deadlineAtMs,maxExpansions:30000000}),preparationBudget:createBudget({deadlineAtMs,maxExpansions:20000000})});
 ...
 assert.equal(result.state,'complete');assert.equal(selected.fuel.state,'provisional_station_access');assert.equal(result.search.poolComplete,true);assert.ok(result.candidates.every(c=>c.fuel==='provisional_station_access'),'Every base candidate must finish fuel search');
