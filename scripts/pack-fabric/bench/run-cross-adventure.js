...
for(const usable of [333000,162000])for(const allowUnknown of [false,true])for(const reverse of [false,true])for(const profile of ['dirt','balanced','clean']) {
 ...
 const result=buildRideAlternatives({input,pack,geom,stations,expandedCandidates:true,maxFuelLabels:100000,fuelHeuristicWeight:2,revision:JSON.stringify(identity),context,budget:createBudget({deadlineAtMs,maxExpansions:30000000}),preparationBudget:createBudget({deadlineAtMs,maxExpansions:20000000})});
