...
const PROVINCES = {
  bc: {
    ...
    // OSM-only BC test: Layers "View BC" paints adventure OSM (track/path/resource),
    // not DRA. Revert sourceMatch to DRA when shipping osm+provincial again.
    sourceMatch: /OpenStreetMap/i,
    sourceLabel: "OpenStreetMap (track / path / adventure — no footway)",
...
function main() {
  ...
  for (const e of graph.edges || []) {
    ...
    if (cfg.adventureOnly) {
      // Keep dual-sport capillary visible; drop highway basemap (already on tiles).
      const rt = String(e.rt || "").toLowerCase();
      const surface = String(surfaceName[e.s] || "").toLowerCase();
      const adventureRt = /^(track|resource|recreation|double_track)$/.test(rt);
      const adventureSurface = /^(resource|access|track|double_track|gravel)$/.test(surface);
      if (!adventureRt && !(rt === "local" && adventureSurface)) continue;
    }
    const coords = e.g || [];
