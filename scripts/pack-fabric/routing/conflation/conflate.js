 ...
 *   When the backbone edge is paved (typically OSM), it owns that corridor —
 *   conventional provincial parallels (gravel/paved locals) are dropped even
 *   outside the tight duplicate window. True capillary (track/resource/access)
 *   still fills between roads.
...
/** Wider window: OSM/NRN paved owns conventional corridor vs NSTDB gravel locals. */
const PAVED_CORRIDOR_METERS = 90;
...
function conflateRegion(options = {}) {
  ...
  for (const edge of supplement) {
    ...
    // OSM/NRN paved owns the physical corridor. Provincial gravel/paved "same
    // road" copies often sit 40–90 m off the OSM centerline (beyond the tight
    // 28 m duplicate window) and make Dirt hop black↔orange every few hundred m.
    // Keep true capillary (track/resource/access); drop conventional parallels.
    const pavedOwner = findNearDuplicate(edge, out, index, {
      maxMeters: PAVED_CORRIDOR_METERS,
      requireSurface: "paved"
    });
    if (pavedOwner && !isProvincialSupplement(edge)) {
      stats.supplementConventionalSkipped += 1;
      bump(reasons, "osm_paved_owns_corridor");
      continue;
    }
  ...
  return {
    ...
    report: {
      ...
      precedence: {
        ...
        pavedCorridor:
          "When backbone surface is paved, conventional provincial parallels within ~90 m are dropped (OSM/NRN paved owns the corridor)",
        supplement:
      ...
      notes: [
        ...
        "Paved backbone owns conventional corridor (~90 m) — provincial gravel/paved locals skipped.",
...
function findNearDuplicate(candidate, backboneEdges, index, options = {}) {
  const maxMeters = options.maxMeters != null ? options.maxMeters : DUPLICATE_METERS;
  const requireSurface = options.requireSurface || null;
  ...
  for (const idx of nearbyIndices(index, mid)) {
    ...
    if (requireSurface && other.surfaceClass !== requireSurface) continue;
    ...
    if (Math.min(endA, endB, endC, endD, dist) <= maxMeters) return other;
  }
  return null;
}
