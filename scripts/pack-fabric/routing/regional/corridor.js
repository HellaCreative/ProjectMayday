...
function extractRoadFabricLonghaulGraph(graph, options = {}) {
  ...
  for (const e of graph.edges || []) {
    ...
    if (osmOnly) {
      ...
      // at ingest). Keep honest access — never rewrite motorized_unknown →
      // permissive (phone packs and Allow Unknown must match live).
      keepEdges.push(e);
      continue;
    }
    ...
    if (osmProvincial) {
      ...
      // BC: full FTEN in longhaul OOMs Vercel Hobby (~340MB JSON). Keep DRA
      // province-wide; FTEN only near hub bulbs (phone pack still has full FTEN).
      if (nrn) continue;
      if (osm) {
        const rt = String(e.rt || "");
        if (options.dropService && rt === "service") continue;
      ...
      if (provincial) {
        if (
          options.ftenNearHubsOnly &&
          /forest tenure/i.test(String(src)) &&
          !nearHub(e)
        ) {
          continue;
        }
    ...
    // OSM fabric — keep honest access (Allow Unknown is the legality gate).
    const edge = e;
    const rt = String(e.rt || "");
