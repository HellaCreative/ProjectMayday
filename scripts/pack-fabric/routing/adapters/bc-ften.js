...
/**
 * FTEN adapter — on disk only. Not in SUPPLEMENTS. Do not wire. Do not publish.
 */
...
async function run(options = {}) {
  ...
  for (;;) {
    ...
    if (startIndex % (pageSize * 10) === 0 || startIndex < pageSize * 2) {
      console.error(
        `[bc-ften] fetched start=${startIndex} kept=${features.length} scanned=${scanned}`
      );
    }
