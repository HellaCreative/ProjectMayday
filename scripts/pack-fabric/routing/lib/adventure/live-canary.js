...
function canarySupported(body,kind,environment=process.env) {
 if(environment.DIRT_ADVENTURE_CANARY!=='ns-v1'||body.action||body.locations?.length!==2||!['dirt','balanced','cleanest'].includes(body.profile))return false;
 const o=body.options||{},f=body.fuel||{};
 if(body.accessPolicy?.motorizedPermissive===false)return false;
 if(o.avoidEdgeIds?.length||o.arrivalEdgeId||o.priorEdgeIds?.length||o.maxPathMeters!=null||o.regionalHopMinimumMeters?.length||o.avoidMotorways||o.cleanMetroMultiplier!=null)return false;
...
function routeResponse(segments,profile,identity,diagnostics,point=null) {
 ...
 return withServiceIdentity({status:'complete',profile,geometry,segments:segments.map(s=>({...s,surfaceClass:surfaceKind(s.surfaceLeaf),accessClass:s.accessClass||'motorized_unknown'})),distanceMeters:surface.distanceMeters,
  ...
  warnings:[warning],maneuvers:[],debug:{routingRevision:'adventure-preview-v1',engine:'adventure-shared-candidates',regionIds:['ns'],packIdentity:identity,diagnostics}});
}
function toLiveResponse(pool,body,kind,identity) {
 ...
 const diagnostics={strategy:'adventure-preview-v1',selectedReason:pool.selectedObjective,adventure:{candidates:pool.candidates,search:pool.search,quality:r.qualityAudit,fuelState:r.fuel.state},packIdentity:identity};
 ...
 return {status:'complete',message:warning.message,stops:kept.map(v=>({...v.station,graphMeters:v.atMeters})),routes,graphMeters:routes.map(r=>r.distanceMeters),windowComplete:complete,
  regionIds:['ns'],packIdentity:identity,destinationEscapeMeters:complete?r.fuel.destinationEscape.distanceMeters:null,diagnostics,
...
async function adventureCanaryRequest(body,kind,{environment=process.env,load=null}={}) {
 ...
 if(!resolution.ok||resolution.regionIds.length!==1||resolution.regionIds[0]!=='ns')return null;
 ...
 const data=load?await load(resolution):await (async()=>{
  ...
  const [runtime,fuel]=await Promise.all([loadGraphsForRequest(resolution,{locations:body.locations,profile:body.profile}),loadRegionFuel('ns')]);
