...
function buildRideAlternatives(options) {
 options={...options,costBoundsWorkspace:options.context?.reuseBounds?require('./bounds-workspace').createBoundsWorkspace():null,avoidanceBoundsCache:require('./target-bounds-cache').createTargetBoundsCache({releaseGraph:options.context?.releaseBoundsGraph===true})};
