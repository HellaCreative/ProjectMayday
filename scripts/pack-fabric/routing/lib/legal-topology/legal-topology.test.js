...
test("toll collection preserves road passage through the final compact pack", () => {
  for (const accessTags of [{}, { access: "yes" }, { motor_vehicle: "yes" }]) {
    for (const tags of [
      ...
      const { pack, geom } = packed({
        nodes: [node(1, -63, 46), node(2, -62.99, 46, { barrier: "toll_booth", ...accessTags }), node(3, -62.98, 46)],
...
test("a toll booth does not override an explicit closure", () => {
  for (const restriction of [{ access: "private" }, { motorcycle: "no" }, { locked: "yes" }, { access: "destination" }, { access: "customers" }]) {
    const { pack } = packed({
