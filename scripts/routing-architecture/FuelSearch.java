...
final class FuelSearch {
 ...
 static final class Label {
  final int node,in,stops;final double remaining,cost,meters;final Label parent;final Step step;boolean live=true;
  Label(int n,int i,double r,double c,double m,int stops,Label p,Step s){node=n;in=i;remaining=r;cost=c;meters=m;this.stops=stops;parent=p;step=s;}
 }
 ...
 static Result search(Graph graph,Weighting raw,List<Integer> starts,Set<Integer> goals,Map<Integer,String> stations,double full,double initial,long deadline,int maxLabels) {
  ...
  PriorityQueue<Label> queue=new PriorityQueue<>(Comparator.comparingDouble((Label l)->l.cost).thenComparingInt(l->l.stops).thenComparingDouble(l->l.meters));
  Map<Long,List<Label>> frontier=new HashMap<>();int labels=0,expanded=0;
  for(int node:starts){Label l=new Label(node,-1,initial,0,0,0,null,null);queue.add(l);frontier.computeIfAbsent(state(node,-1),k->new ArrayList<>()).add(l);labels++;}
  ...
  while(!queue.isEmpty()){
   ...
   if(stations.containsKey(l.node)&&l.remaining<full-1e-6){
    ...
    if(offer(frontier,queue,next)&&++labels>maxLabels)return failed("label_budget",labels,expanded);
   }
 ...
 static boolean offer(Map<Long,List<Label>> fronts,PriorityQueue<Label> queue,Label n){
