...
const {pavedBackroadCost}=require('../pack-fabric/routing/lib/adventure/paved-backroad-cost');
...
for(const record of records){const p=record.result,errors=[],arcs=[];let meters=0;const surface={paved:0,dirt:0,unknown:0};
 for(const edge of p.edges||[]){const key=edge.sourceKey,e=key>>1,reverse=key%2;meters+=edge.meters;const expected=surfaceKind(pack.enums.surfaceLeafNames[pack.edgeSurfaceLeaf[e]]);surface[expected]+=edge.meters;if(edge.surfaceKind!==undefined&&edge.surfaceKind!==({paved:0,dirt:1,unknown:2}[expected]))errors.push('surface mismatch');
  if(edge.pavedBackroadCost!==undefined){const wanted=pavedBackroadCost({distanceMeters:edge.meters,surfaceLeaf:pack.enums.surfaceLeafNames[pack.edgeSurfaceLeaf[e]],roadClassLeaf:pack.enums.roadClassLeafNames[pack.edgeRoadClassLeaf[e]]});if(Math.abs(wanted-edge.pavedBackroadCost)>0.01)errors.push('backroad cost mismatch');}
  if(arcs.at(-1)?.key===key && arcs.at(-1).distanceMeters+edge.meters<=pack.edgeMeters[e]+2)arcs.at(-1).distanceMeters+=edge.meters;else arcs.push({key,id:e,from:reverse?pack.edgeTo[e]:pack.edgeFrom[e],to:reverse?pack.edgeFrom[e]:pack.edgeTo[e],distanceMeters:edge.meters});
 }
