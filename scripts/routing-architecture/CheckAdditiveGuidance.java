...
public final class CheckAdditiveGuidance {
 public static void main(String[] args) throws Exception {
  int checked=0,found=0,combined=0;
   ...
   for(var name:List.of("dirt10","dirt30","paved")) {
    ...
    for(var q:VerifiedHopper.JSON.readTree(Path.of(args[2]).toFile()))for(double lambda:new double[]{30,300}){
     ...
     for(var a:from)for(var b:to){
      ...
      if(expected.isFound())minimum=Math.min(minimum,expected.getWeight());
     ...
     var merged=h.searchEndpoints(graph,w,name,lambda,from.stream().map(x->x.getClosestNode()).distinct().toList(),to.stream().map(x->x.getClosestNode()).distinct().toList(),5000);
     if(!merged.complete()||merged.path().isFound()!=Double.isFinite(minimum)||merged.path().isFound()&&Math.abs(merged.path().getWeight()-minimum)>1e-6*Math.max(1,minimum))throw new AssertionError("Combined endpoint minimum differs from exhaustive Dijkstra pairs");
     combined++;
    }
   }
  }
  System.out.println("CHECK_RESULT "+VerifiedHopper.JSON.writeValueAsString(Map.of("checked",checked,"combined",combined,"found",found,"state","passed","reference","edge-based Dijkstra","populatedLandmarks",true)));
 }
}
