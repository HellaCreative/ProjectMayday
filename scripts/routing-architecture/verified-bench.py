...
if a.artifact:cmd[-1]=a.artifact
...
try:
 ...
 for i in range(a.repeat):
  for profile in a.profiles:
   ...
   query['scaledDistanceGuidance']=a.scaled_distance_guidance;query['downstreamFuelRepair']=a.downstream_fuel_repair;query['earlyFuelRepair']=a.early_fuel_repair;query['objectiveFuelRepair']=a.objective_fuel_repair
   ...
   if a.hybrid:
    envelope=result;result=dict(envelope.get('fuel',{}) if 'fuel' in query else envelope.get('road',{}));result['resources']=envelope.get('resources');result['hybridState']=envelope.get('state');result['hybridError']=envelope.get('error');result['roadCandidate']=envelope.get('road')
 ...
 report={'fixture':fixture,'dataset':a.dataset,'hybrid':a.hybrid,'initializationSeconds':locals().get('initialization'),'initialResidentBytes':locals().get('initialRss'),'scope':'Verified graph, additive objectives, directed turns, paired endpoint directions. City/highway avoidance, full candidate ranking and fuel NOT qualified. Process-cold is not OS-cold.','runs':[{k:v for k,v in x.items() if k!='result'}|{'result':{k:v for k,v in x['result'].items() if k not in ['edges','points','steps','escape','roadSourceKeys','roadCandidate']},'proofSha256':hashlib.sha256(json.dumps(x['result'].get('steps',x['result'].get('edges')),sort_keys=True).encode()).hexdigest()} for x in results]};report['buildIdentitySha256']=build_identity;report['command']=cmd;out.write_text(json.dumps(report,indent=2))
