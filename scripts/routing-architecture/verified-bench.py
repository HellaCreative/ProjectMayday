...
try:
 ...
 for i in range(a.repeat):
  for profile in a.profiles:
   ...
   if result.get('errors') or result.get('error') or ('fuel' in query and result.get('state')!='found'):raise RuntimeError('incomplete route')
finally:
 child.terminate()
 try:child.wait(10)
