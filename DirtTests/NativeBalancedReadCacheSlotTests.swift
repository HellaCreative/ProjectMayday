...
@Suite("Native Balanced read cache slot equivalence", .serialized)
struct NativeBalancedReadCacheSlotTests {
    @Test func nativeRoutesMatchWith256And4096ReadCacheSlots() throws {
        for profile: RouteProfile in [.balanced] {
    ...
    private func route(profile: RouteProfile,seed: UInt64,enabled: Bool) throws -> OnDeviceRouter.Result {
        ...
        router.balancedLabelReadCacheSlots = enabled ? 4096 : 256
