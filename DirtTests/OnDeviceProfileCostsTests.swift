...
struct CrossPackSeamTests {
    ...
    @Test func concatenatingAddsDirtAndPavedMeters() throws {
        ...
        hop2.terminalContinuation = NativeRoutingContinuation(version: 1, sourceEpoch: "transport-test",
            ...
            restrictionContext: [], activeRestrictions: [])
        hop1.searchMeta = OnDeviceRouter.SearchMeta(timedOut: true, pass2Outcome: "timeCap", pops: 17, elapsedMs: 20)
        hop2.searchMeta = OnDeviceRouter.SearchMeta(pass2Outcome: "balancedObjectiveBound", pops: 23, elapsedMs: 30)
        hop1.matchedStart = .init(sourceEpoch: "transport-test", location: .node(10))
        hop1.matchedEnd = .init(sourceEpoch: "transport-test", location: .node(11))
        hop2.matchedStart = .init(sourceEpoch: "transport-test", location: .node(11))
        hop2.matchedEnd = .init(sourceEpoch: "transport-test", location: .node(12))
        let merged = try #require(OnDeviceRouter.Result.concatenating([hop1, hop2]))
        #expect(merged.matchedStart == hop1.matchedStart)
        #expect(merged.matchedEnd == hop2.matchedEnd)
        #expect(merged.matchedEnd != hop1.matchedEnd)
        #expect(merged.searchMeta.timedOut)
        #expect(merged.searchMeta.pass2Outcome == "timeCap;balancedObjectiveBound")
        #expect(merged.searchMeta.pops == 40)
        #expect(merged.searchMeta.elapsedMs == 50)
        let response = RouteResponse(onDevice: merged, priorEdgeIDs: [])
        let decoded = try JSONDecoder().decode(RouteResponse.self, from: JSONEncoder().encode(response))
        #expect(decoded.status == "complete")
        #expect(decoded.hasLimitedRouteSearch)
        #expect(decoded.debug?.searchMeta?.pops == 40)
        #expect(decoded.warnings?.first?.code == "route_search_limited")
        #expect(!RouteResponse(onDevice: hop2, priorEdgeIDs: []).hasLimitedRouteSearch)
