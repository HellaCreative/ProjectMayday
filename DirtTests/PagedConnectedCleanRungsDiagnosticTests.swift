...
@Suite(.serialized)
struct PagedConnectedCleanRungsDiagnosticTests {
    ...
    @Test func currentNSRecordedParentEndToBorderNativeCleanRungs() throws {
        ...
        for run in 0..<2 {
            let measurement = RoutingMeasurement(metadata: [
                "workload":"current-ns-recorded-node-paged-connected-native-clean-rungs",
            ...
            var passEvidence: [[String:Any]] = [], fallbackDisclosure: [String] = []
                        ...
                        try RoutingWorkContext.$deadline.withValue(RoutingWorkContext.limitedDeadline(milliseconds:18_000)) {
                            try reader.withQuery(cancelled:cancelled) { query in try reader.withLegalQuery(cancelled:cancelled) { legal in
                                ...
                                try policy.withPolicy(query:query) { source in
                                    for pass in [ConnectedCleanStage.CleanPass.pavedWall,.unpavedWall,.pavedUrban,.unpavedUrban] {
                                        try RoutingWorkContext.check()
                                        var context = HopSearchContext.forProfile(.cleanest,seed:3511091208)
                                        context.pavedOnly = pass.pavedOnly; context.cityWall = pass.cityWall
                                        context.urbanCoreFallback = !pass.cityWall
                                        context.settlementWall = false; context.settlementFallback = true
                                        context.avoidMotorways = true
                                        let began = ProcessInfo.processInfo.systemUptime
                                        do {
                                            let candidate = try ConnectedCleanStage.calculate(view:view,
                                                origin:.init(pack:0,node:start,matchedEdge:initial),
                                                destination:.init(pack:0,node:end,matchedEdge:destinationEdge),
                                                profile:.cleanest,maxMeters:207_000,context:context,pass:pass,
                                                ridePreferences:preferences,policySources:[source])
                                            passEvidence.append(["pass":pass.rawValue,"outcome":"legal-incumbent",
                                                "seconds":ProcessInfo.processInfo.systemUptime-began,
                                                "limitation":candidate.limitation as Any? ?? NSNull()])
                                            result = candidate; fallbackDisclosure = pass.disclosure; break
                                        } catch ConnectedCleanStage.Failure.passExhausted {
                                            passEvidence.append(["pass":pass.rawValue,"outcome":"exhausted-recorded-pass",
                                                "seconds":ProcessInfo.processInfo.systemUptime-began])
                                        } catch {
                                            passEvidence.append(["pass":pass.rawValue,"outcome":"unproved:\(error)",
                                                "seconds":ProcessInfo.processInfo.systemUptime-began])
                                            throw error // Neither caps nor data failures authorize another rung.
                                        }
                                    }
                                    if result == nil { throw ConnectedCleanStage.Failure.passExhausted }
                                }
                            } }
                        }
                    }
                    outcome = result?.limitation.map { "legal-incumbent-limited:\($0)" } ?? "complete-recorded-clean-stage"
            ...
            var evidence: [String:Any] = ["measurement":try JSONSerialization.jsonObject(with:JSONEncoder().encode(report)),
                ...
                "nativeCleanPasses":passEvidence,"fallbackDisclosure":fallbackDisclosure,
            ...
            let output = fm.temporaryDirectory.appendingPathComponent("paged-connected-clean-rungs-ns-run\(run).json")
            try JSONSerialization.data(withJSONObject:evidence,options:[.sortedKeys]).write(to:output)
            print("[paged-connected-clean-rungs-ns] outcome=\(outcome) evidence=\(output.path)")
            if let result {
