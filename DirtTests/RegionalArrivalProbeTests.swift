...
struct RegionalArrivalProbeTests {
    ...
    @MainActor @Test(arguments: [RouteProfile.dirt, .balanced]) func currentSeam6952913085PrefixAndTail(profile: RouteProfile) async throws {
        ...
        let prepared = packs
        let measurement = RoutingMeasurement(metadata: ["case":"seam6952913085-initial-to-NB-pump", "release":release,"profile":"\(profile)","qualification":"diagnostic-only"])
        ...
        try await RoutingWorkContext.$measurement.withValue(measurement) {
            let results = await RoutingWorkContext.detachedSearch {
                RoutingWorkContext.$deadline.withValue(RoutingWorkContext.limitedDeadline(milliseconds: 20_000)) {
                    ...
                    let prefix = router.routeDetailed(from: initial,to: seam,profile: profile,allowUnknown: false,
                        arrivalEdgeId: "w792623056:135070:135061",arrivalContinuation: arrival,
                        sessionSeed: 3511091208,maxRouteMeters: 207_000,avoidMotorways: false)
                    guard case .success(let route) = prefix,let token = route.terminalContinuation else {
                        return (prefix,Optional<Swift.Result<OnDeviceRouter.Result,OnDeviceRouter.Failure>>.none)
                    }
        ...
        let artifact = fm.temporaryDirectory.appendingPathComponent("regional-arrival-6952913085-\(profile).json")
        try JSONEncoder().encode(Evidence(manifestSHA256: verifiedManifestHashes,preferences: preferences,initialArrival: arrival,seed: 3511091208,usableMeters: 207_000,outcome: outcome,prefix: prefixResponse,tail: tailResponse)).write(to: artifact)
