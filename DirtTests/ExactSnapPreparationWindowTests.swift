...
@Suite("Raw snap preparation windows")
struct ExactSnapPreparationWindowTests {
    @Test("Coordinate collisions and long geometries preserve exact index body", arguments: [false,true])
    func exactBody(doubles: Bool) throws {
        ...
        let starts = [0,32768,8192]
        for edge in 0..<edges {
            put(Int32(starts[edge]),140+4*edge,&graph);put(Int32(starts[edge]+1),152+4*edge,&graph)
            for node in [starts[edge],starts[edge]+1] {
