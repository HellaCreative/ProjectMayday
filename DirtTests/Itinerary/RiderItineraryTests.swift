...
struct RiderItineraryTests {
    ...
    @Test func movingMiddlePointMintsOnlyChangedRoadLegSeedsWhenFuelIsOff() {
        let initial = itinerary([point(0), point(1), point(2), point(3), point(4)])
        let originalSeeds = initial.legs.map(\.routeSeed)

        let change = reduce(
            initial,
            .move(
                waypointID: initial.waypoints[2].id,
                to: RouteCoordinate(longitude: -61.75, latitude: 46.1)
            ),
            automaticFuelEnabled: false
        )

        #expect(change.rebuildFromLegIndex == 1)
        #expect(change.rebuildThroughLegIndex == 2)
        #expect(change.itinerary.legs[0].routeSeed == originalSeeds[0])
        #expect(change.itinerary.legs[1].routeSeed != originalSeeds[1])
        #expect(change.itinerary.legs[2].routeSeed != originalSeeds[2])
        #expect(change.itinerary.legs[3].routeSeed == originalSeeds[3])
    }

    @Test func movingMiddlePointMintsTheIncomingSuffixWhenFuelIsOn() {
        let initial = itinerary([point(0), point(1), point(2), point(3), point(4)])
        let originalSeeds = initial.legs.map(\.routeSeed)
        ...
        let change = reduce(
            ...
            automaticFuelEnabled: true
        )

        #expect(change.rebuildFromLegIndex == 1)
        #expect(change.rebuildThroughLegIndex == nil)
        #expect(change.itinerary.legs[0].routeSeed == originalSeeds[0])
        for index in 1..<change.itinerary.legs.count {
            #expect(change.itinerary.legs[index].routeSeed != originalSeeds[index])
        }
    }

    @Test func insertingOnMiddleStageMintsBothHalvesAndHonorsFuelScope() throws {
        let initial = itinerary([point(0), point(1), point(3), point(4)])
        let originalSeeds = initial.legs.map(\.routeSeed)
        let splitID = initial.legs[1].id

        let roadOnly = reduce(
            initial,
            .insert(afterLegID: splitID, coordinate: point(2)),
            automaticFuelEnabled: false
        )
        #expect(roadOnly.rebuildFromLegIndex == 1)
        #expect(roadOnly.rebuildThroughLegIndex == 2)
        #expect(roadOnly.itinerary.legs[0].routeSeed == originalSeeds[0])
        #expect(roadOnly.itinerary.legs[1].routeSeed != originalSeeds[1])
        #expect(roadOnly.itinerary.legs[2].routeSeed != originalSeeds[1])
        #expect(roadOnly.itinerary.legs[3].routeSeed == originalSeeds[2])
        ...
        #expect(withFuel.rebuildFromLegIndex == 1)
        #expect(withFuel.rebuildThroughLegIndex == nil)
        #expect(withFuel.itinerary.legs[0].routeSeed == originalSeeds[0])
        #expect(withFuel.itinerary.legs[3].routeSeed != originalSeeds[2])
    }

    @Test func profileRebuildKeepsSeedButFuelReplacementMintsTheAffectedLeg() throws {
        let initial = itinerary([point(0), point(1), point(2)], profile: .balanced)
        let selected = initial.legs[1]

        let profileChange = reduce(initial, .setProfile(legID: selected.id, .dirt))
        #expect(profileChange.itinerary.legs[1].routeSeed == selected.routeSeed)
        #expect(profileChange.itinerary.legs[0].routeSeed == initial.legs[0].routeSeed)

        let fuelChange = reduce(
            profileChange.itinerary,
            .setFuelStopOverride(
                legID: selected.id,
                departureAnchorID: selected.from.uuidString,
                stationID: "alternate-pump"
            )
        )
        #expect(fuelChange.itinerary.legs[1].routeSeed != selected.routeSeed)
        #expect(fuelChange.itinerary.legs[0].routeSeed == initial.legs[0].routeSeed)
    }

    @Test func storedRouteSeedsAreNonzeroAndSurviveCodableReplay() throws {
        let original = itinerary([point(0), point(1), point(2)], profile: .dirt)
        #expect(original.legs.allSatisfy { $0.routeSeed > 0 })
        ...
        #expect(replay.legs.map(\.routeSeed) == original.legs.map(\.routeSeed))
    }
