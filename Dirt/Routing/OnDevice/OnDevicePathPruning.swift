...
nonisolated enum OnDevicePathPruning {
    ...
    /// Collapse sharp out-and-backs on the painted polyline (heading reverse > ~150°).
    /// Geographic cell pruning misses some mid-edge snap and parallel-OSM doubles.
    static func stripHeadingReversals(
        _ edges: [EdgePiece],
        minTurnDegrees: Double = 150
    ) -> [EdgePiece] {
        guard !edges.isEmpty else { return edges }
        var flat: [(edgeIndex: Int, coord: CLLocationCoordinate2D)] = []
        for (ei, edge) in edges.enumerated() {
            for c in edge.coords {
                if let last = flat.last,
                   abs(last.coord.latitude - c.latitude) < 1e-9,
                   abs(last.coord.longitude - c.longitude) < 1e-9 {
                    continue
                }
                flat.append((ei, c))
            }
        }
        guard flat.count >= 3 else { return edges }

        var kept: [(edgeIndex: Int, coord: CLLocationCoordinate2D)] = [flat[0]]
        for i in 1..<flat.count {
            let cand = flat[i]
            // Drop intermediate vertices that create a hard U-turn.
            while kept.count >= 2 {
                let a = kept[kept.count - 2].coord
                let b = kept[kept.count - 1].coord
                let c = cand.coord
                let ab = haversineMeters(a, b)
                let bc = haversineMeters(b, c)
                if ab < 2 || bc < 2 { break }
                let turn = abs(headingDeltaDegrees(from: a, via: b, to: c))
                if turn < minTurnDegrees { break }
                // U-turn at b — remove b and re-check against the new tip.
                kept.removeLast()
            }
            if let tip = kept.last,
               abs(tip.coord.latitude - cand.coord.latitude) < 1e-9,
               abs(tip.coord.longitude - cand.coord.longitude) < 1e-9 {
                continue
            }
            kept.append(cand)
        }
        guard kept.count >= 2 else { return edges }

        var rebuilt: [EdgePiece] = []
        var currentEdge = kept[0].edgeIndex
        var buf: [CLLocationCoordinate2D] = [kept[0].coord]
        for pt in kept.dropFirst() {
            if pt.edgeIndex != currentEdge {
                if let piece = clonePiece(edges[currentEdge], coords: buf) {
                    rebuilt.append(piece)
                }
                currentEdge = pt.edgeIndex
                // Stitch joint onto next edge so we don't open a gap.
                buf = [pt.coord]
                if let last = rebuilt.last?.coords.last {
                    buf = [last, pt.coord]
                }
            } else {
                buf.append(pt.coord)
        ...
        return rebuilt.isEmpty ? edges : rebuilt
    }

    private static func headingDeltaDegrees(
        from a: CLLocationCoordinate2D,
        via b: CLLocationCoordinate2D,
        to c: CLLocationCoordinate2D
    ) -> Double {
        let h1 = bearingDegrees(a, b)
        let h2 = bearingDegrees(b, c)
        var d = h2 - h1
        while d > 180 { d -= 360 }
        while d < -180 { d += 360 }
        return d
    ...
    private static func bearingDegrees(_ a: CLLocationCoordinate2D, _ b: CLLocationCoordinate2D) -> Double {
